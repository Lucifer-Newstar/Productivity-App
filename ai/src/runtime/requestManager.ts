...
export class RequestManager {
  ...
  constructor(private readonly orchestrator: IntelligenceOrchestrator, private readonly maximumActive: number, private readonly toolTimeoutMs = 30_000) {}
  ...
  create(input: IntelligenceRequestInput, sessionId: string): string {
    ...
    this.requests.set(id, state); this.emit(state, { type: "request.started", requestId: id, at: new Date().toISOString() });
    void this.orchestrator.run(input, sessionId, id, (request, signal) => this.waitForTool(state, request, signal), (event) => this.emit(state, event), state.controller.signal)
      .then((response) => { state.status = "complete"; this.emit(state, { type: "response.completed", response, at: new Date().toISOString() }); })
      .catch((error: unknown) => {
        if (state.controller.signal.aborted) { state.status = "cancelled"; this.emit(state, { type: "request.cancelled", at: new Date().toISOString() }); return; }
        state.status = "failed"; const known = error instanceof IntelligenceError ? error : new IntelligenceError("ENGINE_FAILED", "Kaizen Intelligence encountered a local error.", true);
  ...
  private emit(state: RequestState, event: EngineEvent): void { state.events.push(event); if (state.events.length > 500) state.events.shift(); for (const listener of state.listeners) listener(event); }
}
