...
The package is documented under [`v0.1.1-model-evaluation/`](v0.1.1-model-evaluation/README.md). `I1-SYNTHETIC-1`, the production-path runner, scorer, sanitizer, semantic worksheet and harness QA are implemented. `I1-PREFLIGHT-AUTH-1` permits target preflight only in frozen order; no result exists, full/operations remain blocked and Wave 0 stays closed.
