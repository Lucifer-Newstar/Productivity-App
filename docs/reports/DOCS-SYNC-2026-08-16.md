...
The complete `docs/` tree was checked against source on branch `entertainment` after AFTERGLOW v1.0 and the post-release security audit. That branch was subsequently merged into `main` through pull request #2 on 2026-08-17.
...
Dated bug entries and `docs/quality/qa/TEST-REPORT.md` preserve old measurements intentionally. They now carry visible historical-snapshot notices. Current release claims come from:
