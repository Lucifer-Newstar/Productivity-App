...
export async function GET(request:NextRequest){
 ...
 const provider=request.nextUrl.searchParams.get("provider")??"",id=request.nextUrl.searchParams.get("id")??"",type=request.nextUrl.searchParams.get("type") as MediaType;
 ...
 try{return NextResponse.json({result:await getEntertainmentDetails(provider,id,type,requestCredentials(request))},{headers:{"Cache-Control":"private, max-age=300"}})}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"metadata refresh failed"},{status:error instanceof ProviderError?error.status:500})}
}
