...
export async function GET(request:NextRequest){
 ...
 try{return NextResponse.json({results:await trendingEntertainment(type)},{headers:{"Cache-Control":"private, max-age=300"}})}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"discovery failed",provider:error instanceof ProviderError?error.provider:undefined},{status:error instanceof ProviderError?error.status:500})}
}
