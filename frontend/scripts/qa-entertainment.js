...
test('dashboard KPIs present',page.includes('t("inMotion")')&&page.includes('t("timeInside")'));
...
const i18n=read('lib/entertainmentI18n.ts');
test('English Tamil and Hindi locales exist',i18n.includes('id:"en"')&&i18n.includes('id:"ta"')&&i18n.includes('id:"hi"'));
test('localized nav and hero copy exists',i18n.includes('heroText')&&i18n.includes('நூலகம்')&&i18n.includes('लाइब्रेरी'));
test('language selector updates persisted settings',page.includes('ENTERTAINMENT_LANGUAGES')&&page.includes('settings:{...s.settings,language'));
test('provider locale is sent on search/trending/detail',page.includes('&lang=${encodeURIComponent(language)}')&&page.includes('entertainment.settings.language)'));
test('TMDB and Google adapters consume locale',providers.includes('providerLocale(language)')&&providers.includes('langRestrict=${encodeURIComponent(language)}'));
test('provider caches are isolated by language',providers.includes('trending:${language}:${type}')&&providers.includes('${language}:${type}:${q.toLowerCase()}'));
test('mobile navigation exposes all ready sections',page.includes('aria-label="Entertainment sections"')&&page.includes('VIEW_NAV.filter(n=>n.ready)'));
test('skip link and focus target exist',page.includes('Skip to content')&&page.includes('id="ent-main"'));
test('dialogs expose modal semantics',page.includes('role="dialog"')&&page.includes('aria-modal="true"'));
test('Escape closes overlays and details',page.includes('e.key!=="Escape"')&&page.includes('setSelected(null)'));
