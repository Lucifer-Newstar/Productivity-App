...
check("client keeps token in sessionStorage and authorization header",client.includes("sessionStorage")&&client.includes("authorization:`Bearer"));check("same-origin proxy pins loopback gateway",proxy.includes("KAIZEN_AI_GATEWAY_URL")&&proxy.includes("127.0.0.1")&&proxy.includes("sameHost")&&proxy.includes("KAIZEN_AI_PROXY_ORIGIN")&&!proxy.includes("request.nextUrl.searchParams.get(\"url\")"));check("Intelligence UI is explicitly local and read-only",panel.includes("LOCAL · READ ONLY")&&panel.includes("One-time pairing code"));
