...
const fs=require("fs"),path=require("path"),repo=path.resolve(__dirname,"../.."),roots=["frontend","backend","ai"],extensions=new Set([".ts",".tsx",".js",".mjs",".py",".ps1",".css",".sh"]),ignored=new Set(["node_modules",".next","dist","build","__pycache__"]),files=[];
...
const pattern=/(^|\n)\s*(?:\/\/|\/\*|#(?!\!)|"""|'''|<\#)/,missing=files.filter(file=>!pattern.test(fs.readFileSync(file,"utf8")));
