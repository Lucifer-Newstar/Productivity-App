...
check("backup contains every authoritative key",Object.keys(backup.data).length===AUTHORITATIVE_KEYS.length&&AUTHORITATIVE_KEYS.every(key=>key in backup.data)&&AUTHORITATIVE_KEYS.includes("kaizen.profile"));
...
const legacy=JSON.parse(text);legacy.schemaVersion=1;delete legacy.data["kaizen.profile"];check("schema v1 backups without profile still parse",parseBackup(JSON.stringify(legacy)).schemaVersion===1);
