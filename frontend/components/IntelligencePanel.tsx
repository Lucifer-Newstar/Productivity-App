...
export default function IntelligencePanel({buildSnapshot}:{buildSnapshot:()=>TodaySnapshot}){
 ...
 return <section className="intelligence-console home-panel" aria-labelledby="intelligence-title">
  ...
  <div className="intelligence-body"><div className="intelligence-quick"><button onClick={()=>void run()} disabled={state==="busy"}><Sparkles size={12}/>Interpret today&apos;s focus</button><button className="disconnect" onClick={disconnect} aria-label="Disconnect Intelligence Engine"><Unplug size={13}/></button></div><p className="intelligence-scope">Kaizen selects the read-only Core Today evidence. The interpreter cannot choose tools or access other domains.</p>
   {state==="busy"&&<button className="intelligence-cancel" onClick={cancel} aria-label="Cancel interpretation"><X size={14}/>Cancel</button>}
   {state==="busy"&&!response&&<div className="intelligence-loading"><LoaderCircle size={16}/><span>{streaming?"Validating a grounded response…":"Reading current Core Today evidence…"}</span></div>}
