...
export default function Pomodoro(){
 const [mode,setMode]=useState<Mode>("focus"),[secondsLeft,setSecondsLeft]=useState(1500),[running,setRunning]=useState(false),[sessions,setSessions]=useState(0),[muted,setMuted]=useState(true); const intervalRef=useRef<NodeJS.Timeout|null>(null);
 ...
 const reset=()=>{setSecondsLeft(total);setRunning(false)}, ModeIcon=MODES[mode].icon,circ=2*Math.PI*142;
 return <div className="core-section core-focus">
  ...
  <section className={`focus-console mode-${mode} ${running?"is-running":""}`}>
   ...
   <div className="focus-stage">
    ...
    <aside className="focus-protocol"><div><ShieldCheck/><span>Session rule</span><strong>Protect one outcome</strong><p>Close unrelated tabs. Silence inbound alerts. Write down distractions instead of following them.</p></div><div><Activity/><span>Today</span><strong>{sessions} cycles closed</strong><p>{sessions*25} focused minutes recorded in this browser session.</p></div></aside>
   </div>
   <div className="focus-controls"><button onClick={reset}><RotateCcw/>Reset</button><button className="focus-primary" onClick={()=>setRunning(r=>!r)}>{running?<Pause/>:<Play/>}{running?"Pause protocol":"Begin focus"}</button><button onClick={()=>setSecondsLeft(0)}><SkipForward/>Skip phase</button></div>
  </section>
 </div>
}
