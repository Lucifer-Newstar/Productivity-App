...
import {
  ...
  Compass, Film, Heart, Library, LoaderCircle, Moon, PenLine, Plus, Search, Sparkles, Star,
  Sun, Trash2, Wifi, X, Zap,
} from "lucide-react";
import { useStore } from "../../lib/store";
import { useTheme } from "../../lib/theme";
import { readSafeImageAsDataUrl } from "../../lib/security";
import type { EntertainmentEventType, EntertainmentItem, EntertainmentView, MediaPriority, MediaSearchResult, MediaStatus, MediaType } from "../../lib/entertainmentTypes";
...
function AddModal({onClose,onAdd}:{onClose:()=>void;onAdd:(i:EntertainmentItem)=>void}){
 ...
 const [query,setQuery]=useState("");const [results,setResults]=useState<MediaSearchResult[]>([]);const [loading,setLoading]=useState(false);const [error,setError]=useState("");const [coverDataUrl,setCoverDataUrl]=useState<string|undefined>();
 const makeItem=(base:Partial<EntertainmentItem>&Pick<EntertainmentItem,"title"|"type">):EntertainmentItem=>{const now=Date.now();return {id:uid(),provider:"manual",genres:[],creators:[],cast:[],studios:[],status,progress:{},repeats:0,priority,queueOrder:now,tags:[],favorite:false,archived:false,createdAt:now,updatedAt:now,...base}};
 const submit=()=>{if(!title.trim())return;onAdd(makeItem({type,title:title.trim(),description:description.trim()||undefined,coverDataUrl,releaseYear:year?Number(year):undefined,genres:genres.split(",").map(x=>x.trim()).filter(Boolean),creators:creator.split(",").map(x=>x.trim()).filter(Boolean),tags:tags.split(",").map(x=>x.trim()).filter(Boolean)}));};
 const searchApi=async()=>{if(query.trim().length<2)return;setLoading(true);setError("");setResults([]);try{const r=await fetch(`/api/entertainment/search?q=${encodeURIComponent(query.trim())}&type=${type}`);const data=await r.json();if(!r.ok)throw new Error(data.error||"Search failed");setResults(data.results??[]);}catch(e){setError(e instanceof Error?e.message:"Search failed");}finally{setLoading(false)}};
 const importResult=(r:MediaSearchResult)=>onAdd(makeItem({provider:r.provider,providerId:r.providerId,type:r.mediaType,title:r.title,originalTitle:r.originalTitle,description:r.description,coverUrl:r.coverUrl,releaseDate:r.releaseDate,releaseYear:r.releaseYear,genres:r.genres,creators:r.creators,cast:r.cast,studios:r.studios,language:r.language,minutesConsumed:0,progress:{totalPages:r.totalPages,totalChapters:r.totalChapters,totalVolumes:r.totalVolumes,totalEpisodes:r.totalEpisodes,totalSeasons:r.totalSeasons}}));
 return <motion.div className="fixed inset-0 z-[80] bg-black/75 backdrop-blur-sm grid place-items-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={onClose}><motion.div onMouseDown={e=>e.stopPropagation()} initial={{scale:.96,y:16}} animate={{scale:1,y:0}} exit={{scale:.96}} className="ent-panel rounded-3xl w-full max-w-3xl p-6 max-h-[90vh] overflow-auto"><div className="flex"><div><div className="text-xs tracking-[.22em] text-fuchsia-400">QUICK ADD</div><h2 className="text-2xl font-black">Add to the archive</h2></div><button onClick={onClose} className="ml-auto"><X/></button></div>
 <div className="flex gap-2 mt-5 p-1 rounded-xl border w-fit" style={{borderColor:"var(--line)"}}><button onClick={()=>setMode("search")} className={`px-4 py-2 rounded-lg text-xs font-bold flex gap-2 ${mode==="search"?"bg-fuchsia-500 text-white":""}`}><Wifi size={13}/> CATALOGUE SEARCH</button><button onClick={()=>setMode("manual")} className={`px-4 py-2 rounded-lg text-xs font-bold flex gap-2 ${mode==="manual"?"bg-fuchsia-500 text-white":""}`}><PenLine size={13}/> MANUAL</button></div>
 {mode==="search"?<div className="mt-5"><div className="grid sm:grid-cols-[160px_1fr_auto] gap-2"><Filter value={type} onChange={v=>{setType(v as MediaType);setResults([]);setError("")}} options={Object.entries(MEDIA_TYPE_LABELS)}/><div className="relative"><Search size={14} className="absolute left-3 top-3" style={{color:"var(--muted)"}}/><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchApi()} placeholder="Search provider catalogue…" className="input pl-9"/></div><button onClick={searchApi} disabled={loading||query.trim().length<2} className="px-5 py-2 rounded-xl bg-cyan-500 text-[#041016] font-black text-xs disabled:opacity-40">{loading?<LoaderCircle size={16} className="animate-spin"/>:"SEARCH"}</button></div>
 <div className="text-[10px] mt-2" style={{color:"var(--muted)"}}>{type==="anime"?"MyAnimeList":type==="manga"?"AniList":type==="movie"||type==="series"?"TMDB":type==="comic"?"Comic Vine → Books fallback":"Google Books → Open Library fallback"}</div>
 {error&&<div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-300 p-3 text-xs">{error}</div>}
 <div className="grid sm:grid-cols-2 gap-3 mt-5">{results.map(r=><button key={`${r.provider}:${r.providerId}`} onClick={()=>importResult(r)} className="text-left rounded-2xl border overflow-hidden hover:border-fuchsia-400/60 transition flex min-h-28" style={{borderColor:"var(--line)"}}>{r.coverUrl?<img src={r.coverUrl} alt="" className="w-20 object-cover shrink-0"/>:<div className="w-20 grid place-items-center shrink-0" style={{background:`${TYPE_COLORS[r.mediaType]}22`,color:TYPE_COLORS[r.mediaType]}}>{MEDIA_TYPE_ICONS[r.mediaType]}</div>}<div className="p-3 min-w-0"><div className="text-[9px] uppercase tracking-wider" style={{color:TYPE_COLORS[r.mediaType]}}>{r.provider}</div><div className="font-bold text-sm line-clamp-2">{r.title}</div><div className="text-[10px] mt-1 truncate" style={{color:"var(--muted)"}}>{[r.releaseYear,...r.creators].filter(Boolean).join(" · ")||"Metadata available"}</div><div className="text-[10px] text-fuchsia-400 mt-2">+ ADD TO LIBRARY</div></div></button>)}</div>{!loading&&!error&&query&&results.length===0&&<div className="text-center py-8 text-xs" style={{color:"var(--muted)"}}>Search to see catalogue matches, or switch to Manual.</div>}</div>
 :<><p className="text-xs mt-4" style={{color:"var(--muted)"}}>Create a private local entry when a provider has no match.</p><div className="grid md:grid-cols-2 gap-3 mt-5"><Field label="Title *"><input autoFocus value={title} onChange={e=>setTitle(e.target.value)} className="input"/></Field><Field label="Media type"><Filter value={type} onChange={v=>setType(v as MediaType)} options={Object.entries(MEDIA_TYPE_LABELS)}/></Field><Field label="Status"><Filter value={status} onChange={v=>setStatus(v as MediaStatus)} options={STATUS.map(s=>[s.id,s.label])}/></Field><Field label="Priority"><Filter value={priority} onChange={v=>setPriority(v as MediaPriority)} options={[["high","High"],["medium","Medium"],["low","Low"]]}/></Field><Field label="Creator / author / director"><input value={creator} onChange={e=>setCreator(e.target.value)} className="input" placeholder="Comma separated"/></Field><Field label="Release year"><input type="number" min="1800" max="2100" value={year} onChange={e=>setYear(e.target.value)} className="input"/></Field><Field label="Genres"><input value={genres} onChange={e=>setGenres(e.target.value)} className="input" placeholder="Drama, Sci-Fi"/></Field><Field label="Mood / custom tags"><input value={tags} onChange={e=>setTags(e.target.value)} className="input" placeholder="cozy, weekend"/></Field><Field label="Cover image (JPEG/PNG/WebP, max 2 MB)"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={async e=>{const f=e.target.files?.[0];if(!f)return;try{setCoverDataUrl(await readSafeImageAsDataUrl(f));setError("")}catch(err){setError(err instanceof Error?err.message:"Invalid image")}}} className="text-xs"/></Field>{coverDataUrl&&<div className="rounded-xl border p-2 flex items-center gap-2" style={{borderColor:"var(--line)"}}><img src={coverDataUrl} alt="cover preview" className="w-10 h-14 object-cover rounded"/><span className="text-[10px] text-emerald-400">Cover ready</span></div>}<div className="md:col-span-2"><Field label="Description"><textarea rows={3} value={description} onChange={e=>setDescription(e.target.value)} className="input resize-none"/></Field></div></div><div className="flex justify-end gap-2 mt-6"><button onClick={onClose} className="px-4 py-2 text-sm">Cancel</button><button onClick={submit} disabled={!title.trim()} className="px-5 py-2 rounded-xl text-sm font-bold bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white disabled:opacity-40">ADD TITLE</button></div></>}
 <style jsx>{`.input{width:100%;border:1px solid var(--line);background:transparent;border-radius:12px;padding:9px 11px;font-size:13px;outline:none}`}</style></motion.div></motion.div>
}
