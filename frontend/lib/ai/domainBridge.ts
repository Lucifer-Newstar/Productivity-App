...
export function buildTodaySnapshot(input:{tasks:Task[];forgeTasks:Array<{id:string;title:string}>;notifications:KaizenNotification[];intelligence:HomeIntelligence;now?:Date;tracker:BridgeRevisionTracker}):TodaySnapshot{
  const now=input.now??new Date(),localDate=day(now),active=input.tasks.filter(t=>!t.completed).slice(0,50),today=input.intelligence.today.filter(x=>!x.done).slice(0,25);
  ...
  const attention=input.intelligence.attention.filter((x:any)=>x.priority==="high"||x.priority==="critical").slice(0,25).map((x:any)=>({notificationId:String(x.id??x.sourceKey??idFor("notice",`${x.section}|${x.title}`)),section:String(x.section??"global"),priority:x.priority as "high"|"critical",title:String(x.title)}));
