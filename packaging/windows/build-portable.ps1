# Builds a reproducible Windows x64 portable directory and ZIP with a checksum-pinned Node runtime.
[CmdletBinding()]
...
function Run([string]$File, [string[]]$Arguments, [string]$Directory) {
...
New-Item $frontend -ItemType Directory -Force | Out-Null
...
$intelligence = Join-Path $stage "intelligence"
New-Item $intelligence -ItemType Directory -Force | Out-Null
...
$hash = (Get-FileHash $zip -Algorithm SHA256).Hash.ToLowerInvariant(); Set-Content "$zip.sha256" "$hash  $(Split-Path $zip -Leaf)"
