...
[CmdletBinding()]
param([string]$Version="1.0.0",[string]$Iscc="${env:ProgramFiles(x86)}\Inno Setup 6\ISCC.exe",[switch]$SkipStage)
$ErrorActionPreference="Stop"
if(-not $SkipStage){& (Join-Path $PSScriptRoot "stage-package.ps1") -Version $Version;if($LASTEXITCODE -ne 0){throw "Package staging failed"}}
