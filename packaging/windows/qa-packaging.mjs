...
test("desktop errors append to LOCALAPPDATA Kaizen desktop-error.log",()=>assert.ok(desktop.includes('function errorLogFile')&&desktop.includes('appendFileSync')&&desktop.includes('desktop-error.log')&&desktop.includes('%LOCALAPPDATA%')));
...
test("delivery allocates the next stable patch release after green main CI",()=>{const workflow=read(".github/workflows/windows-installer.yml");assert.ok(workflow.includes('patch + 1')&&workflow.includes('gh release list')&&workflow.includes('GH_TOKEN: ${{ github.token }}')&&workflow.includes('github.event.workflow_run.conclusion')&&workflow.includes('concurrency:'))});
test("delivery emits sanitized manifest before stable publication",()=>{const workflow=read(".github/workflows/windows-installer.yml");assert.ok(workflow.includes('kaizen.windows-delivery-manifest')&&workflow.includes('PUBLIC-SANITIZED-AGGREGATE')&&workflow.includes('release-manifest.json')&&workflow.includes("Publish stable patch release"))});
...
test("green main CD publishes an immutable tagged stable release",()=>{const workflow=read(".github/workflows/windows-installer.yml");assert.ok(workflow.includes('git tag -a "$tag" "$DELIVERY_SHA"')&&workflow.includes('gh release create "$tag" release-artifacts/*')&&workflow.includes('--generate-notes'))});
