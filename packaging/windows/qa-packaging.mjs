...
test("delivery allocates the next stable patch release after green main CI",()=>{const workflow=read(".github/workflows/windows-installer.yml");assert.ok(workflow.includes('patch + 1')&&workflow.includes('gh release list')&&workflow.includes('github.event.workflow_run.conclusion')&&workflow.includes('concurrency:'))});
